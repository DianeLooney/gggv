...
void main() {
  ...
  float x, y;
  float stride = 3;
  for (x = fragTexCoord.x - dxSearchSamples * dx;
       x <= fragTexCoord.x + dxSearchSamples * dx; x += dx) {
    for (y = fragTexCoord.y - dySearchSamples * dy;
         y <= fragTexCoord.y + dySearchSamples * dy; y += dy) {
